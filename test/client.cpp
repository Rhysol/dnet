...
public:
	...
	virtual void OnNewConnection(int32_t, const std::string &, uint16_t) override
	{
	}
    virtual void OnReceivePacket(int32_t, const NetPacketInterface &, uint32_t) override
	{
		++m_count;
	}
...
void thread_func(uint32_t thread_id, uint32_t total_send_num)
{
	...
	for (uint32_t i = 0; i < total_send_num; i++)
	{
		...
		if (0 == i)
		{
            net.Send(connection_fd, start_data.data(), start_data.max_size());
		}
		else if (total_send_num - 1 == i)
		{
            net.Send(connection_fd, end_data.data(), end_data.max_size());
		}
		else
		{
